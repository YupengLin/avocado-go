...
class AvocadoGoAPI {
  ...
  ///api/v1/eth/collection/trending_1_month
  String avocadoBaseUrl =
      "http://Solmar-env.eba-vxgpfxub.us-east-1.elasticbeanstalk.com/api";

  AvocadoGoAPI(this.url) {
    if (url == null) return;
  ...
  Uri? _getOAuthURL(String requestMethod, String endpoint, version) {
    ...
    var token = '';
    var url = '${this.url!}/ad-json/wc/v2/$endpoint';
    // Default one is v3
    if (version == 3) {
  ...
  Future<dynamic> getAsyncV2(
    String endPoint, {
    int version = 2,
    bool enableDio = false,
    bool refreshCache = true,
  }) async {
    final url = '$avocadoBaseUrl$endPoint'.toUri();
    var response = await httpCache(url!, refreshCache: refreshCache);
    //var response = await httpCache(url!, refreshCache: true);
    return json.decode(response.body);
  }
  ...
  Future<dynamic> postAsync(String endPoint, Map data,
    ...
    printLog(
        '[avocado_api][${DateTime.now().toString().split(' ').last}] postAsync START [endPoint:$endPoint] url:$url');
    ...
    printLog(
        '[avocado_api][${DateTime.now().toString().split(' ').last}] postAsync END [endPoint:$endPoint]');
    return dataResponse;
  }
